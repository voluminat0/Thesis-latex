...
%Readability is an inevitable consequence of expressiveness in a DSL. Implementing fluent
%interfaces is one way to improve readability and make Smart APIs. You can implement
%method chaining so that the output from one method flows naturally as the
%input of another. This technique makes your series of API invocations feel more natural,
%and it’s closer to the sequence of actions that you would perform in the problem
...
%to write. Dynamic builders like the ones in Groovy or Ruby smooth out the implementation
%aspect of your DSL by constructing methods dynamically through the
...
%make DSLs expressive using the power of a type system
...
%-> types maken meer consise en robust
...
%-> Kweni of dit er in moet
%newline separators
%-> Language supports newlines between every state
%method chaining/function sequence/fluent interface
%-> Bread and butter of our DSL
%macro
%-> Used when using user-specified/defined predicates
%literal map
%-> Argument is a map similar to literal map
%closure
%-> prop/cond -> in literal map
%object scoping
%-> Alles wordt geevalueerd tegen 1 object om global namespace pollution tegen te gaan (alsook security issues)
%state machine
%-> Houd lijst van states bij die apart geprocessed worden
